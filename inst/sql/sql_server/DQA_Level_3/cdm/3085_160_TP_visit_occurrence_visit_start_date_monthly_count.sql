...
--
select * into #visit_year_and_month_statics from @resultSchema.dq_check_result
where check_id = 'C160'
--
...
select
  ...
  ,p1.p_90
from
  (select check_id, stratum1, stratum2, stratum5, count_val from #visit_year_and_month_statics)as s1
left join
(select distinct
	 stratum2
  ...
  from #visit_year_and_month_statics) as p1
...
left join
  (select
		 ...
		 from #visit_year_and_month_statics
