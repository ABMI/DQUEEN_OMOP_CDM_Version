...
,'check the procedure_occurrence table quantity per visit_type' as stratum5
,count_val
,null as num_val
,null as txt_val
from
...
--
select * into #procedure_year_and_month_statics from @resultSchema.dq_check_result
where check_id = 'C293'
--
...
select
  ...
  ,p1.p_90
from
  (select check_id, stratum1, stratum2, stratum3, stratum4,stratum5, count_val from #procedure_year_and_month_statics)as s1
left join
(select distinct
	 stratum4
  ...
  from #procedure_year_and_month_statics) as p1
...
left join
  (select
		 ...
		 from #procedure_year_and_month_statics
