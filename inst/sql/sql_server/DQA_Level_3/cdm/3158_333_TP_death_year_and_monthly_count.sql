...
from
(select
substring(convert(varchar,death_date,121),1,7) as death_date
from @cdmSchema.death)v
group by death_date
order by death_date;
--
select * into #death_statics from @resultSchema.dq_check_result
where check_id = 'C334'
--
...
select
  ...
  ,s1.stratum1
  ,s1.stratum2
  ,'deah_date of death table year and monthly count' as stratum3
  ...
  ,p1.p_90
from
  (select check_id, stratum1, stratum2, stratum5, count_val from #death_statics)as s1
left join
(select distinct
	 stratum2
