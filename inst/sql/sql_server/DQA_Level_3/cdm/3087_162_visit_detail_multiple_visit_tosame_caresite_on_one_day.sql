...
group by  person_id,visit_detail_concept_id,visit_detail_start_date,care_site_id)v ;
--
select * into #multi_visit_count from @resultSchema.dq_check_result
where check_id = 'C162'
--
...
select distinct
  ...
  ,p1.p_90
from
  (select check_id, stratum1, stratum2, stratum4, count_val from #multi_visit_count) as s1
left join
(select distinct
	 stratum2
	,stratum4
