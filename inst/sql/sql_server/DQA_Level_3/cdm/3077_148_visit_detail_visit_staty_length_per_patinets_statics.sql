 ...
 select
  ...
  (
    ...
    (select
		...
		,case
			 when visit_detail_end_date is null then 1
			 else datediff(day,visit_detail_start_date,visit_detail_end_date)+1
			end as diff
			from @cdmSchema.visit_detail)w)v;
--
select
 stratum4
,stratum5
into #statics_visit_detail
...
from  #statics_visit_detail ) as p1
