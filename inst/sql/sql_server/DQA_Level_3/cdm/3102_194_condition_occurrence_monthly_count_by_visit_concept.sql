...
,condition_month as stratum4
,'initial visit_concept monthly count of condition_occurrence ' as stratum5
,count(*) as count_val
,null as num_val
,null as txt_val
from
...
group by visit_concept_id,condition_month
--
select * into #condition_occurrence_visit_monthly_statics from @resultSchema.dq_check_result
where check_id = 'C194'
--
...
select
  ...
  ,p1.p_90
from
  (select check_id, stratum1, stratum3, stratum4,stratum5, count_val from #condition_occurrence_visit_monthly_statics)as s1
left join
(select distinct
	 stratum3
  ...
  from #condition_occurrence_visit_monthly_statics) as p1
...
left join
  (select
		 ...
		 from #condition_occurrence_visit_monthly_statics
