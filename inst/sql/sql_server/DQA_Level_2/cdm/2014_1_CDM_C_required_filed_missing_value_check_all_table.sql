...
Use @cdm;  --> meta db nameΩ
--
IF OBJECT_ID('tempdb..##missingcheck', 'U') IS NOT NULL
   DROP TABLE ##missingcheck
create table ##missingcheck(
       tbnm nvarchar(255)
      ,colnm nvarchar(255)
      ,err_no nvarchar(255) )
  --
...
set @tbnm= (select stuff(( select stratum3+',' from @resultSchema.dq_check_result
                              where check_id = 'C1' and txt_val = 'Y'
                                and stratum1 in (2,7,12,13,14,15,16,18,19,20,23,24,26,27,28,29,30,31,32,33,36,37,38)
                           for xml path('')),1,0,''))
       print(@tbnm)
...
begin
        ...
        print(@tbnm1)
           set @colnm= (select stuff(( select c1.stratum4+',' from
                          (select stratum1, stratum3, stratum4 from @resultSchema.dq_check_result
                            where check_id = 'C2' and stratum2 = 'cdm' and txt_val= 'Y' and stratum3 = ''+@tbnm1+'') as c1
                          inner join (select tb_id, tbnm, col_id, colnm from @resultSchema.schema_info where stage_gb = 'CDM'
                                      and (is_nullable = 'No' or check_gb = 'Y')) as s1
   ...
   if @colnm1 is not null
           begin
               ...
               set @sql1='insert into ##missingcheck select * from (select '+''''+@tbnm1+''''+' as tbnm , '+''''+@colnm1+''''+' as colnm ,'+@colnm2+' as err_no from '+@tbnm1 +
                         ' where cast('+@colnm1+' as varchar) is null or cast('+@colnm1+' as varchar)= '+ ''''+''''+')v'
                   print(@sql1)
                   exec(@sql1)
               end
       else
