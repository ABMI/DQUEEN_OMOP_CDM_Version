...
--
IF OBJECT_ID('tempdb..##pecheck', 'U') IS NOT NULL
   DROP TABLE ##pecheck
create table ##pecheck(
       tbnm nvarchar(255)
      ,colnm nvarchar(255)
      ,err_no nvarchar(255) )
  --
...
begin
   ...
   if @colnm1 is not null
           begin
               ...
               set @sql1=' insert into ##pecheck '+
                         ' select '+
                         ''''+@tbnm1+''''+' as tbnm '+
                         ', '+''''+@colnm1+''''+' as colnm '+
                         ', '+@colnm2+' as err_no '+
                         ' from '+@tbnm1+
                         ' where person_id in ( select person_id from '+@temp_cdmSchema+'person '+
                         ' where person_source_value in ( select patno from '+@temp_metaSchema+'dev_person '+
                         ' where include_yn = '+''''+'N'+''''+')'+')'
                   print(@sql1)
                   exec(@sql1)
               end
...
,null as stratum4
,'check the patients records to be excluded' as stratum5
,count(*) as count_val
,null as num_val
,null as txt_val
from
