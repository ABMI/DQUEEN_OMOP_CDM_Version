...
--
IF OBJECT_ID('tempdb..##invalidprovider', 'U') IS NOT NULL
   DROP TABLE ##invalidprovider
create table ##invalidprovider(
       tbnm nvarchar(255)
      ,colnm nvarchar(255)
      ,err_no nvarchar(255) )
  --
...
begin
           ...
           set @colnm= (select stuff(( select c1.stratum4+',' from
                          (select stratum1, stratum3, stratum4 from @resultSchema.dq_check_result
                            where check_id = 'C2' and stratum2 = 'cdm' and txt_val= 'Y' and stratum3 = ''+@tbnm1+''
                                  and stratum4= 'provider_id') as c1
                        for xml path('')),1,0,''))
       print(@colnm)
   ...
   if @colnm1 is not null
           begin
               ...
               set @sql1=' insert into ##invalidprovider '+
                         ' select '+
                         ''''+@tbnm1+''''+' as tbnm '+
                         ', '+''''+@colnm1+''''+' as colnm '+
                         ', '+@colnm2+' as err_no '+
                         ' from '+@tbnm1+
                         ' where provider_id in ( select provider_id from '+@temp_cdmSchema+'provider '+
                         ' where provider_source_value in ( select userid from '+@temp_metaSchema+'dev_provider '+
                         ' where include_yn = '+''''+'N'+''''+')'+')'
                   print(@sql1)
                   exec(@sql1)
               end
...
,null as stratum4
,'check the provider_id records to be excluded' as stratum5
,count(*) as count_val
,null as num_val
,null as txt_val
from
